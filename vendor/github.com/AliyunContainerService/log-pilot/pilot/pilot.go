...
func (p *Pilot) parseLogConfig(name string, info *LogInfoNode, jsonLogPath string, mounts map[string]types.MountPoint,logpatternconfig map[string]string) (*LogConfig, error) {
	log.Debugf("549 info--- ", info)
	path := strings.TrimSpace(info.value)
	if strings.ToUpper(name) == strings.ToUpper("multiline") || strings.ToUpper(name) == strings.ToUpper("pattern") {
		return nil, fmt.Errorf("552 key : %s ", path)
	}
	if path == "" {
		return nil, fmt.Errorf("字段 %s 空", name)
	}

	tags := info.get("tags")
	tagMap, err := p.parseTags(tags)
	if err != nil {
		return nil, fmt.Errorf("分析标签 %s 错误: %v", name, err)
	}
	...
	// 多行处理
	multiline := logpatternconfig[strings.ToUpper("multiline")]
	pattern := ""
	if	multiline != "" && strings.ToUpper(multiline) == "TRUE" {
		pattern = logpatternconfig[strings.ToUpper("pattern")]
		if pattern == "" {
			pattern = ""
			multiline = ""
		}
	}
	log.Debugf(" 610 multiline %v pattern %v " , multiline, pattern)
	...
	if !filepath.IsAbs(path) {
		return nil, fmt.Errorf("%s 必须是绝对路径, for %s", path, name)
	}
...
// {value: xxx, children: {value: xxx, children: {value: children:{}}}}
type LogInfoNode struct {
	value    string `json:"value"`
	children map[string]*LogInfoNode `json:"children"`
}
...
func (node *LogInfoNode) insert(keys []string, value string) error {
	log.Debugf(" 679 LogInfoNode ---",keys, value)
	if len(keys) == 0 {
		return nil
	}
	key := keys[0]
...
func (p *Pilot) getLogConfigs(jsonLogPath string, mounts []types.MountPoint, labels map[string]string) ([]*LogConfig, error) {
	...
	logpatternconfig := make(map[string]string)
	...
	for _, k := range labelNames {
		for _, prefix := range p.logPrefix {
			...
			log.Debugf("748 logLabel :", logLabel, "labels[k] :", labels[k])
			if strings.ToUpper(logLabel)  == strings.ToUpper("multiline") || strings.ToUpper(logLabel) == strings.ToUpper("pattern") {
				logLabel_pattern := strings.ToUpper(logLabel)
				logpatternconfig[logLabel_pattern] = labels[k]
			}
	...
	for name, node := range root.children {
		if strings.ToUpper(name) == strings.ToUpper("multiline") || strings.ToUpper(name) == strings.ToUpper("pattern") {
			...
			logConfig, err := p.parseLogConfig(name, node, jsonLogPath, mountsMap,logpatternconfig)
			if err != nil {
				return nil, err
			}
